...
def add_family(email, family_email):
    """adds family members to the login menu or
    proceeds to the menu when no other family members"""
    profiles = db.child('Profiles').get()
    for profile in profiles.each():
        if profile.val()['email'] == family_email:
            # Add existing member
